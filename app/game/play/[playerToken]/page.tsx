...
import { getPlayerByToken, formatTime, getRemainingTime, isGameOver, getGreekWord } from '@/lib/gameSession';
import type { Player } from '@/lib/gameSession';
...
export default function PlayerGamePage() {
  ...
  const { player: initialPlayer } = getPlayerByToken(playerToken);
  const player = initialPlayer;
  const loading = !initialPlayer;
  ...
  }, [initialPlayer, router]);

  useEffect(() => {
    if (!player) return;

    const interval = setInterval(() => {
      const { session } = getPlayerByToken(playerToken);
      if (session) {
        const remaining = getRemainingTime(session);
        setTimeRemaining(remaining);

        if (isGameOver(session) && player.status !== 'completed') {
          clearInterval(interval);
          router.replace(`/game/gameover/${playerToken}`);
        }
      }
    }, 1000);

    return () => clearInterval(interval);
  }, [player, playerToken, router]);
  ...
  const getPhase = () => {
    if (player.status === 'completed') return 'complete';
    if (player.bonusCompleted) return 'complete';
    if (player.miniGameCompleted) return 'bonus';
    if (player.meaningCompleted) return 'mini-game';
    if (player.morseCompleted) return 'meaning';
    return 'morse';
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {phase === 'morse' && (
          <div className="space-y-6">
            ...
            <MorsePuzzle player={player} playerToken={playerToken} showHelp={showHelp} setShowHelp={setShowHelp} />
          </div>
        )}
        ...
        {phase === 'meaning' && (
          <div className="space-y-6">
            ...
            <MeaningPuzzle player={player} playerToken={playerToken} />
          </div>
        )}
